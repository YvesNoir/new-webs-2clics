-- Migración de SQLite a PostgreSQL para new-webs-2clics
-- Conectarse a la base de datos: \c "db_new2clics"
...
-- Tabla de tasaciones (nueva funcionalidad)
...
VALUES
    ('hero-block-default', 'hero', 1, true, '{}', 'default', NOW(), NOW()),
    ('ultimas-prop-1761345231', 'featured-properties', 3, true, '{}', 'default', NOW(), NOW()),
    ('barrios-populares-1761345409', 'popular-neighborhoods', 4, true, '{}', 'default', NOW(), NOW())
ON CONFLICT ("id") DO NOTHING;
...
CREATE INDEX IF NOT EXISTS "idx_site_blocks_siteConfigId" ON "site_blocks"("siteConfigId");
CREATE INDEX IF NOT EXISTS "idx_properties_agentId" ON "properties"("agentId");
CREATE INDEX IF NOT EXISTS "idx_properties_status" ON "properties"("status");
CREATE INDEX IF NOT EXISTS "idx_properties_type" ON "properties"("type");
CREATE INDEX IF NOT EXISTS "idx_properties_featured" ON "properties"("featured");
CREATE INDEX IF NOT EXISTS "idx_contacts_propertyId" ON "contacts"("propertyId");
CREATE INDEX IF NOT EXISTS "idx_contacts_status" ON "contacts"("status");
CREATE INDEX IF NOT EXISTS "idx_tasaciones_status" ON "tasaciones"("status");
...
-- Comentarios para campos específicos nuevos
...
BEGIN
    ...
    -- Verificar que existen todas las tablas esperadas
    IF EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'new2clics_schema'
        AND table_name IN ('admins', 'site_configs', 'site_blocks', 'agents', 'properties', 'contacts', 'tasaciones')
    ) THEN
        RAISE NOTICE 'Todas las tablas fueron creadas exitosamente';
    ELSE
        RAISE NOTICE 'Algunas tablas no fueron creadas correctamente';
    END IF;
...
/*
PASOS PARA USAR ESTE SCRIPT EN pgAdmin:

1. Conectarse a la base de datos "db_new2clics"
2. Ejecutar este script completo
3. Verificar que todas las tablas se crearon
4. Actualizar el archivo .env con la nueva configuración:

   DATABASE_URL="postgresql://app_new2clics:New2clics7211.@localhost:5432/db_new2clics?schema=new2clics_schema"

5. Ejecutar: npx prisma generate
6. Ejecutar: npx prisma db push

DATOS INCLUIDOS:
- Usuario admin: admin@inmobiliaria.com / password configurado
- Configuración básica del sitio con colores del footer
- 2 agentes de ejemplo
- 4 propiedades de ejemplo
- Bloques del sitio configurados

NUEVAS FUNCIONALIDADES INCLUIDAS:
- Configuración personalizada del footer (colores y logo)
- Sistema de tasaciones completo
- Todas las configuraciones de páginas dinámicas
- Sistema completo de redes sociales
*/